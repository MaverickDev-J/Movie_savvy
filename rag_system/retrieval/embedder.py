...
def update_metadata(chunk_to_embedding_info):
    ...
    try:
        ...
        if isinstance(metadata, dict) and 'items' in metadata:
            # Metadata has the structure {"items": [...]}
            items = metadata['items']
        elif isinstance(metadata, list):
            # Metadata is directly a list of items
            items = metadata
        else:
            logger.error(f"Unexpected metadata structure: {type(metadata)}")
            sys.exit(1)
        
        # Update metadata with embedding information
        for item in tqdm(items, desc="Updating metadata"):
            item['embedding_info'] = {}
