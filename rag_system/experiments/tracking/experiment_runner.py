...
#     """Manages the execution of experiments for the rag_system.
    
#     Handles parameter sweeps, query execution, and logging to MLflow.
#     """
...
#         """Run a specific type of experiment.
...
#         # Parent run for the experiment type
...
#                 with self.mlflow_manager.start_run(
#                     run_name=f"params_{i+1}",
#                     tags={'param_combination': str(param_dict)}
#                 ):
#                     self.mlflow_manager.log_params(param_dict)
                    
#                     # Track metrics across all queries for this parameter set
#                     combo_metrics = []
                    
#                     for query_type, queries in test_queries.items():
#                         for query_idx, query in enumerate(queries):
#                             # Grandchild run for each query
#                             with self.mlflow_manager.start_run(
#                                 run_name=f"{query_type}_{query_idx}",
#                                 tags={
...
#                                     self.mlflow_manager.log_metrics({'error_occurred': 1})
                    
#                     # Log aggregated metrics for this parameter combination
#                     if combo_metrics:
#                         aggregated_metrics = self._aggregate_metrics(combo_metrics)
#                         self.mlflow_manager.log_metrics(aggregated_metrics)
    
#     async def _run_single_query(self, query: str, query_type: str, parameters: Dict[str, Any], experiment_type: str) -> Dict[str, float]:
...
#             vector_chunks = await self.retrieve_vector(query, top_k)
...
#         # Add error rate
#         error_count = len(metrics_list) - len(valid_metrics)
#         aggregated['error_rate'] = error_count / len(metrics_list)
