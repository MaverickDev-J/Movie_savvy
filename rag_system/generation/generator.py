...
# YOUTUBE INSIGHTS (video content and discussions):
# {youtube_context}
...
#     def _format_context(self, context_list: list, context_type: str) -> str:
#         """Format context with better structure and filtering."""
#         if not context_list:
#             return f"No {context_type.lower()} available."
...
#             text = ctx.get('content', '') if isinstance(ctx, dict) else ctx
#             if text and len(text.strip()) > 20:  # Filter very short content
#                 cleaned_ctx = text.strip()[:800]  # Limit length
#                 cleaned_context.append(f"{i}. {cleaned_ctx}")
        
#         return "\n".join(cleaned_context) if cleaned_context else f"No relevant {context_type.lower()} available."

#     def _format_youtube_context(self, youtube_context: list, context_type: str) -> str:
#         """Format YouTube context with better structure and filtering."""
#         if not youtube_context:
#             return f"No {context_type.lower()} available."
        
#         cleaned_context = []
#         for i, ctx in enumerate(youtube_context[:5], 1):  # Limit to top 5
...
#         local_context: list[str],
#         web_context: list[str],
#         youtube_context: list[dict] = None,  # Added youtube_context
#         query_analysis: dict = None,
#         max_new_tokens: int | None = None,
#         temperature: float | None = None,
#         top_k: int | None = None,
#     ) -> str:
#         """Enhanced generation with query analysis and YouTube integration."""
#         max_new_tokens = max_new_tokens or rag_config["generation"]["parameters"]["max_new_tokens"]
#         temperature = temperature or rag_config["generation"]["parameters"]["temperature"]
#         top_k = top_k or rag_config["generation"]["parameters"].get("top_k", 50)
...
#             local_context=local_context,
#             web_context=web_context,
#             youtube_context=youtube_context,  # Pass youtube_context
#             current_query=query,
...
#             if query_analysis and query_analysis.get("query_type") == "opinion":
#                 temperature = min(temperature * 1.2, 0.9)
...
#         """Enhanced prompt building with YouTube context integration."""
#         template = self.default_prompt_template
        
#         local_context_str = self._format_context(local_context, "Local Knowledge")
#         web_context_str = self._format_context(web_context, "Web Insights")
#         youtube_context_str = self._format_youtube_context(youtube_context, "YouTube Insights")
        
#         context_priority = "youtube" if (query_analysis and query_analysis.get("needs_youtube")) else "web" if (query_analysis and query_analysis.get("needs_web_search")) else "local"
        
#         return template.format(
#             query=current_query,
#             local_context=local_context_str,
#             web_context=web_context_str,
#             youtube_context=youtube_context_str  # Add YouTube context
#         )
...
### Response:"""
    ...
    def _build_prompt(self, local_context: list[str], web_context: list[str], youtube_context: list[dict], current_query: str, query_analysis: dict = None) -> str:
        ...
        if youtube_context and query_analysis and query_analysis.get("query_type") in ["trailer_request", "fan_discussion", "youtuber_recommendation"]:
            prompt += f"YouTube Trailer Insights (Primary Source):\n{self._format_youtube_context(youtube_context, 'YouTube Insights')}\n\n"
        
        if local_context:
            prompt += f"Local Context (Secondary):\n{self._format_context(local_context, 'Local Context')}\n\n"
        
        if web_context:
            prompt += f"Web Insights (Supplementary):\n{self._format_context(web_context, 'Web Insights')}\n\n"
        
        prompt += "Provide a detailed, accurate response based primarily on YouTube context for trailer-related queries, supplemented by local and web context if needed. Focus on specific trailer details (e.g., scenes, characters, themes) for trailer queries."
