...
class ContextWeighting:
    """Class to dynamically weight contexts based on query intent."""
    
    def __init__(self):
        self.weights = {
            'trailer_request': {'youtube': 0.7, 'web': 0.2, 'local': 0.1},
            'youtuber_recommendation': {'youtube': 0.6, 'web': 0.3, 'local': 0.1},
            'fan_discussion': {'youtube': 0.6, 'web': 0.3, 'local': 0.1},
            'review_analysis': {'youtube': 0.5, 'web': 0.3, 'local': 0.2},
            'behind_scenes': {'youtube': 0.5, 'web': 0.3, 'local': 0.2},
            'comparison': {'youtube': 0.4, 'web': 0.4, 'local': 0.2},
            'reaction': {'youtube': 0.5, 'web': 0.3, 'local': 0.2},
            'explanation': {'youtube': 0.3, 'web': 0.4, 'local': 0.3},
            'other': {'youtube': 0.2, 'web': 0.4, 'local': 0.4}
        }
    
    def get_weights(self, query_type: str) -> Dict[str, float]:
        """Return context weights for the given query type."""
        return self.weights.get(query_type, self.weights['other'])
...
class Generator:
    ...
    def __init__(self):
        ...
        self.context_weighting = ContextWeighting()
